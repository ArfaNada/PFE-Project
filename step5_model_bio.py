#!/usr/bin/env python
"""
Author Bio Sentiment Classification Model

This script trains a Decision Tree classifier on author bio text to predict sentiment
...
def train_bio_model(data_path, output_dir="."):
    """
    Train a sentiment classification model on author bio text.
    ...
    # Create TF-IDF features from author bios
    ...
    # Save the vectorizer
    vectorizer_path = os.path.join(output_dir, 'tfidf_v_final_bio.pkl')
    with open(vectorizer_path, 'wb') as f:
        pickle.dump(tfidf_v, f)
    print(f"Vectorizer saved to {vectorizer_path}")
    
    # Save the model
    model_path = os.path.join(output_dir, 'decision_tree_model_bio.pkl')
...
def load_and_predict(text, vectorizer_path='tfidf_v_final_bio.pkl', 
                     model_path='decision_tree_model_bio.pkl'):
    """
    Load trained model and make predictions on new text.
    
    Args:
        text: Bio text to classify
...
if __name__ == "__main__":
    ...
    # dtc, tfidf_v = train_bio_model(data_path, output_dir)
    
    # Example prediction
    example_text = "I'm a curious explorer who enjoys discovering new things."
    # prediction = load_and_predict(example_text)
    # print(f"Bio: {example_text}")
